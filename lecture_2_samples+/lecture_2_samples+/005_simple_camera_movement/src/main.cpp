...
int main () {
    ...
    while (window.isOpen()) {
		...
		//since rotation is around axis of translation, the order of these two doesn't matter (try it out!)
		//comment them both out, only one or the other, and change the order of these two lines
        cameraMatrix = glm::rotate(cameraMatrix, glm::radians(clock()/10.0f), glm::vec3(0, 0, 1));
        cameraMatrix = glm::translate(cameraMatrix, glm::vec3(0, 0, clock()/1000.0f));
