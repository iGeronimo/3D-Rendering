...
int main () {
    ...
    sf::Window window(sf::VideoMode (800, 600), "OpenGL Window", sf::Style::Default, sf::ContextSettings(24,0,0,3,3));
    ...
    while (window.isOpen()) {
		...
		//camera still at 0,0,0,0
        glm::mat4 cameraMatrix = glm::mat4(1.0f);
        glm::mat4 viewMatrix = glm::inverse (cameraMatrix);
        
		//without model matrix, best we can do is put together the vp part at the moment... (which is actually pv in OpenGL's backward system)
		...
		//NOW just before rendering, we construct model matrix 1
        glm::mat4 modelMatrix1    (glm::translate (glm::vec3(-1,0,-5)) * glm::rotate (clock()/1000.0f, glm::vec3(0.7,0.7,0.7)));
		//...use it to create our first mvp (which is actually pvm in OpenGL backwards system...)
        glUniformMatrix4fv (glGetUniformLocation (programID, "mvpMatrix"), 1, GL_FALSE, glm::value_ptr(vp*modelMatrix1));
		//...aaaand draw it to the screen, woohoo!
        glDrawArrays( GL_TRIANGLES, 0, 36);

		//Now just repeat those steps for the 2nd cube...
        glm::mat4 modelMatrix2    (glm::translate (glm::vec3(1,0,-5)) * glm::rotate (clock()/1000.0f, glm::vec3(0.7,0.7,0.7)));
