...
void main( void ) {
	...
	float specularConstant = pow(max(dot(posToViewDirVec, reflectDirVec), 0), 4);
	vec3 specularFinal = vec3(1.f, 1.f, 1.f) * specularConstant;

	//Attenuation
	float attenuation = diffuse / (1.f + .22f * distance(vs_position, lightPos0) + .02f * (distance(vs_position, lightPos0) * distance(vs_position, lightPos0)));
